...
const retryWithBackoff = async <T>(
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      // Se for erro de quota ou API key inválida, não tentar novamente
      if (error.message?.includes('quota') || 
          error.message?.includes('API key') || 
          error.message?.includes('permission') ||
          error.status === 429 ||
          error.status === 403) {
        throw new Error('Erro de API: Verifique sua chave ou quota do Gemini');
      }
      
      // Última tentativa, lança o erro
      if (i === maxRetries - 1) {
        throw new Error('Falha após várias tentativas. Tente novamente em alguns minutos.');
      }
